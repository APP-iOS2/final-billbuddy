...
class SampleMemeberStore: ObservableObject {    
    ...
    init() {
        let sample = TravelCalculation(hostId: "", travelTitle: "", managerId: "", startDate: Date.now, endDate: Date.now + 1, updateContentDate: Date.now, members: [TravelCalculation.Member(name: "인원1", advancePayment: 200, payment: 0)])
        self.travel = sample
